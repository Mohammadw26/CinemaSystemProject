...
public class SeatChoosingController {
	...
	void bookSeat() {
		...
		if (confLabel.isVisible() && confLabel.getText() != "You must choose a seat to continue.") {
			BookingRequest request = new BookingRequest(temp, temp2, screening, counter,
					screening.getMovie().getTicketCost());
			try {
			...
			confLabel.setText("You have selected the following seats: " + chosenSeats + "\nTotal Cost: "
					+ Double.toString(screening.getMovie().getTicketCost() * counter) + " NIS"
	...
	void selectSeat(MouseEvent event) {
		...
		if (((Node) event.getSource()).getStyle()
			...
			((Node) event.getSource())
					.setStyle("-fx-fill:#792f2f; -fx-font-family: 'Material Icons'; -fx-font-size: 40.0;");
			counter--;
			...
			((Node) event.getSource())
					.setStyle("-fx-fill:#ff0000; -fx-font-family: 'Material Icons'; -fx-font-size: 40.0;");
			counter++;
			chosenNum.setText("Number of chosen seats: " + Integer.toString(counter));
			warning.setVisible(false);
		}
	...
	void initialize() {
		...
		assert availableSeatsLabel != null
				: "fx:id=\"availableSeatsLabel\" was not injected: check your FXML file 'SeatChoosing.fxml'.";
		...
		movieInfo.setText("Movie: " + screening.getMovie().getMovieTitle() + " - "
				+ screening.getMovie().getMovieTitleHeb() + "\nDate: " + screening.getScreeningDate() + "\nTime: "
	...
	private void drawSeats() {
		...
		if ((hall.getCols() / 2) * 2 == (hall.getCols())) {
			...
			for (int k = 0; k < hall.getRows(); k++) {
				...
				for (; i <= j && seatNum < hall.getSeatsNum(); i++) {
					if (!screening.getSeatStatus(seatNum)) {
						gridSeats.getChildren().get(i + k * 7)
								.setStyle("-fx-fill:#792f2f; -fx-font-family: 'Material Icons'; -fx-font-size: 40.0;");
					} else {
						gridSeats.getChildren().get(i + k * 7)
								.setStyle("-fx-fill:#e7d1d1; -fx-font-family: 'Material Icons'; -fx-font-size: 40.0;");
					}
					seatNum++;
				}
			}
	...
	@Subscribe
	public void onSeatSavedEvent(SeatSavedEvent event) {
		Platform.runLater(() -> {
			screening = (Screening) event.getNewScrn();
			initialize();
		});
	}
